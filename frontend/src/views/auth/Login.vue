                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input id="email" v-model="email" name="email" type="email" autocomplete="email" required
                            class="input" :placeholder="$t('authPages.emailPlaceholder')"
...
async function handleSubmit() {
    try {
        ...
        if (!isValidEmail.value) {
            error.value = t('authPages.invalidEmail')
            return
        }

        if (password.value.length < 8) {
            error.value = t('authPages.passwordTooShort')
            return
        }
        ...
        error.value = err.message || t('authPages.signInFailed')
