...
<template>
    <div class="min-h-screen bg-slate-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <!-- Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-900">{{ $t('courseDashboard.title') }}</h1>
                <p class="mt-1 text-gray-500">{{ $t('courseDashboard.subtitle') }}</p>
            </div>
            ...
            <template v-else-if="data">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        ...
                        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">{{
                            $t('courseDashboard.grossRevenue') }}</p>
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        ...
                        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">{{
                            $t('courseDashboard.totalStudents') }}</p>
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        ...
                        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">{{
                            $t('courseDashboard.averageRating') }}</p>
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        ...
                        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">{{
                            $t('courseDashboard.activeAssets') }}</p>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        <h2 class="text-lg font-semibold text-gray-900 mb-6">{{ $t('courseDashboard.acquisitionVolume')
                            }}</h2>

                        <div v-if="!data.acquisitionVolume.length"
                            class="flex items-center justify-center h-48 text-gray-400 text-sm">
                            {{ $t('courseDashboard.noCourseData') }}
                        </div>
                    ...
                    <div class="bg-gray-900 rounded-2xl p-6 text-white flex flex-col">
                        <h2 class="text-lg font-semibold mb-5">{{ $t('courseDashboard.topPerformers') }}</h2>
                        ...
                        <div class="space-y-3 flex-1">
                                ...
                                <div class="flex items-center gap-3 min-w-0">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-xs text-gray-400 uppercase tracking-wider">{{
                                            course.students.toLocaleString() }} {{ $t('courseDashboard.studentsLabel')
                                            }}</p>
                                    </div>
                                </div>
                            ...
                            <div v-if="!data.topPerformers.length" class="text-gray-500 text-sm text-center py-4">
                                {{ $t('courseDashboard.noCoursesYet') }}
                            </div>
                        </div>
                        ...
                        <div v-if="data.averageRating" class="mt-6 pt-5 border-t border-white/10">
                            <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">{{
                                $t('courseDashboard.qualityScore') }}</p>
            ...
            <div v-else-if="error" class="text-center py-24 text-gray-500">
                <p>{{ $t('courseDashboard.loadFailed') }} <button class="text-sky-500 hover:underline" @click="load">{{
                    $t('courseDashboard.retry') }}</button></p>
            </div>
...
const qualityLabel = computed(() => {
    ...
    if (r >= 4.8) return t('courseDashboard.qualityExceptional')
    if (r >= 4.5) return t('courseDashboard.qualityOutstanding')
    if (r >= 4.0) return t('courseDashboard.qualityHighly')
    if (r >= 3.5) return t('courseDashboard.qualityGood')
    return t('courseDashboard.qualityGrow')
})
