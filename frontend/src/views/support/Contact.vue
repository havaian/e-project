<!-- frontend/src/views/support/Contact.vue -->
<template>
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            <div class="grid lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white rounded-xl shadow-lg p-8">
                        <h2 class="text-2xl font-bold text-gray-900 mb-6">{{ $t('contact.getInTouch') }}</h2>
                        ...
                        <div class="mb-8">
                            <div class="flex items-center space-x-3 mb-4">
                                ...
                                <div>
                                    <h3 class="font-semibold text-gray-900">{{ $t('contact.support247') }}</h3>
                                    <p class="text-sm text-gray-600">{{ $t('contact.availableAroundClock') }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-6">
                            <div class="flex items-start space-x-4">
                                ...
                                <div>
                                    <h4 class="font-semibold text-gray-900">{{ $t('contact.phoneSupport') }}</h4>
                                    <a :href="`tel:${supportPhone}`"
                                        class="text-sky-600 hover:text-sky-700 transition-colors">
                                        {{ supportPhone }}
                                    </a>
                                    <p class="text-sm text-gray-600 mt-1">{{ $t('contact.phoneHint') }}</p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-start space-x-4">
                                ...
                                <div>
                                    <h4 class="font-semibold text-gray-900">{{ $t('contact.emailSupport') }}</h4>
                                    <a :href="`mailto:${supportEmail}`"
                                        class="text-purple-600 hover:text-purple-700 transition-colors">
                                        {{ supportEmail }}
                                    </a>
                                    <p class="text-sm text-gray-600 mt-1">{{ $t('contact.emailHint') }}</p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-start space-x-4">
                                ...
                                <div>
                                    <h4 class="font-semibold text-gray-900">{{ $t('contact.liveChat') }}</h4>
                                    <button @click="openLiveChat"
                                        class="text-emerald-600 hover:text-emerald-700 transition-colors">
                                        {{ $t('contact.startConversation') }}
                                    </button>
                                    <p class="text-sm text-gray-600 mt-1">{{ $t('contact.liveChatHint') }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-8 pt-8 border-t border-gray-200">
                            <h3 class="font-semibold text-gray-900 mb-4">{{ $t('contact.departmentExtensions') }}</h3>
                            <div class="space-y-3 text-sm">
                                <div class="flex justify-between">
                                    <span class="text-gray-600">{{ $t('contact.generalSupport') }}</span>
                                    <span class="text-gray-900">ext. 101</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-600">{{ $t('contact.technicalSupport') }}</span>
                                    <span class="text-gray-900">ext. 102</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-600">{{ $t('contact.billingPayments') }}</span>
                                    <span class="text-gray-900">ext. 103</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-600">{{ $t('contact.providerSupport') }}</span>
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-8">
                        ...
                        <form @submit.prevent="submitForm" class="space-y-6">
                            ...
                            <div class="grid md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select id="category" v-model="form.category"
                                        ...
                                        <option value="technical">{{ $t('contact.catTechnical') }}</option>
                                        <option value="billing">{{ $t('contact.catBilling') }}</option>
                                        <option value="provider">{{ $t('contact.catProvider') }}</option>
                                        <option value="accessibility">{{ $t('contact.catAccessibility') }}</option>
                                        <option value="privacy">{{ $t('contact.catPrivacy') }}</option>
                                        <option value="feature">{{ $t('contact.catFeature') }}</option>
                                        <option value="bug">{{ $t('contact.catBug') }}</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-3">{{
                                    $t('contact.priorityLevel') }}</label>
                                <div class="flex space-x-4">
                                    <label class="flex items-center">
                                        <input v-model="form.priority" type="radio" value="low"
                                            class="text-sky-600 focus:ring-sky-500" />
                                        <span class="ml-2 text-sm text-gray-700">{{ $t('contact.priorityLow') }}</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input v-model="form.priority" type="radio" value="medium"
                                            class="text-sky-600 focus:ring-sky-500" />
                                        <span class="ml-2 text-sm text-gray-700">{{ $t('contact.priorityMedium')
                                            }}</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input v-model="form.priority" type="radio" value="high"
                                            class="text-sky-600 focus:ring-sky-500" />
                                        <span class="ml-2 text-sm text-gray-700">{{ $t('contact.priorityHigh') }}</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input v-model="form.priority" type="radio" value="urgent"
                                            class="text-sky-600 focus:ring-sky-500" />
                                        <span class="ml-2 text-sm text-gray-700">{{ $t('contact.priorityUrgent')
                                            }}</span>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div class="flex justify-end">
                                <button type="submit" :disabled="isSubmitting"
                                    ...
                                    <svg v-if="isSubmitting" class="animate-spin w-4 h-4 mr-2" fill="none"
                                        viewBox="0 0 24 24">
                        ...
                        <div v-if="showSuccess" class="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                            <div class="flex items-center">
                                ...
                                <p class="text-green-800">{{ $t('contact.successMessage') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-16">
                <div class="text-center mb-8">
                    <h2 class="text-3xl font-bold text-gray-900 mb-4">{{ $t('contact.faqTitle') }}</h2>
                    <p class="text-gray-600">{{ $t('contact.faqSubtitle') }}</p>
                </div>

                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div class="bg-white rounded-lg shadow-md p-6">
                        <h3 class="font-semibold text-gray-900 mb-2">{{ $t('contact.faqResponseTime') }}</h3>
                        <p class="text-gray-600 text-sm">{{ $t('contact.faqResponseTimeAnswer') }}</p>
                    </div>

                    <div class="bg-white rounded-lg shadow-md p-6">
                        <h3 class="font-semibold text-gray-900 mb-2">{{ $t('contact.faqSchedule') }}</h3>
                        <p class="text-gray-600 text-sm">{{ $t('contact.faqScheduleAnswer') }}</p>
                    </div>

                    <div class="bg-white rounded-lg shadow-md p-6">
                        <h3 class="font-semibold text-gray-900 mb-2">{{ $t('contact.faqCost') }}</h3>
                        <p class="text-gray-600 text-sm">{{ $t('contact.faqCostAnswer') }}</p>
                    </div>
                </div>
            </div>
        </div>
