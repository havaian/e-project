...
exports.uploadProfilePhoto = async (req, res) => {
    try {
        ...
        // Delete old profile picture file if it exists and it's a local file
...
exports.generateAvatar = async (req, res) => {
    try {
        ...
        // Use different avatar generation services
        const avatarServices = [
            // UI Avatars (free, simple) - using hex color
            `https://ui-avatars.com/api/?name=${encodeURIComponent(initials)}&size=200&background=${backgroundHex.replace('#', '')}&color=ffffff&format=png&font-size=0.4`,
            
            // DiceBear (more variety, also free)
            `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(user.firstName + user.lastName)}&backgroundColor=${backgroundHex.replace('#', '')}&textColor=ffffff`,
            
            // Alternative: Gravatar-style (based on email hash)
            // `https://www.gravatar.com/avatar/${crypto.createHash('md5').update(user.email.toLowerCase().trim()).digest('hex')}?s=200&d=identicon`
        ];

        // Use the first service (UI Avatars)
...
/**
 * Serve avatar/profile picture files
 * @param {Object} req - Express request object
 * @param {Object} res - Express response object
 */
exports.getAvatar = async (req, res) => {
    try {
        ...
        // Set cache headers for better performance (optional)
        res.set({
